...
df$log_ctf <- (df$log_rec_total - df$log_desp_total) # resultado total ou CFT
...
df$d_gap_pos # dummy positivo
df$d_gap_neg  # dummy negativo

df$d_pib_gap_pos = df$log_pib_gap * df$d_gap_pos  #  hiato do produto x dummy positivo
df$d_pib_gap_neg = df$log_pib_gap * df$d_gap_neg # hiato do produto  x dummy negativo
...
vars_int <- c("log_rec_total", # fiscais - receita
              "log_rcl",
              "log_rcl_cred", 
